...
output "private_tgw_attachment_subnet_id" {
  value = aws_subnet.private_tgw_attachment.id
}
...
resource "aws_subnet" "private_tgw_attachment" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = cidrsubnet(var.vpc_cidr_block, 8, 2)
  availability_zone       = "ap-northeast-1a"
  map_public_ip_on_launch = true
  tags = {
    Name = "${var.vpc_name}__private_tgw_attachment"
  }
}
...
resource "aws_route_table" "vpc" {
  ...
  route {
    cidr_block         = var.other_vpc_cidr_block
...
resource "aws_route_table_association" "vpc_private_tgw_attachment" {
  subnet_id      = aws_subnet.private_tgw_attachment.id
  route_table_id = aws_route_table.vpc.id
}
